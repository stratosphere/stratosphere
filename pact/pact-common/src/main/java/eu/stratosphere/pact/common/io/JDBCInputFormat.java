...
public class JDBCInputFormat extends GenericInputFormat {
	 ...
	 * found.
	 */
	private boolean setClassForDBType() {
		if (isFieldNullOrEmpty(driverName)) {
			return false;
		}
	 ...
	 * established
	 */
	private boolean prepareConnection(String username, String password) throws SQLException {
		if(isFieldNullOrEmpty(dbURL)) {
			return false;
		}
	...
	public void configure(Configuration parameters) {
		...
		if (needConfigure) {
			this.driverName = parameters.getString("drivername","");
			...
			this.dbURL = String.format("jdbc:%s://%s:%d/%s", dbTypeStr, host, port, dbName);
		}

		prepareQueryExecution();
	}

	private void prepareQueryExecution() throws IllegalArgumentException {
		if (!setClassForDBType()) {
	...
	private void prepareCredentialsAndExecute() throws SQLException {
		if (dbURL == null) {
			if (prepareConnection(username, password)) {
				executeQuery();
			}
		}
		else {
