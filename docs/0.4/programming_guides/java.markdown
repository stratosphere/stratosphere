...
sublinks:
  ...
  - {anchor: "example", title: "Word Count Example"}
...
We now look at a more concrete example, the classical Word Count. The goal of Word Count is to determine the number of occurrences of each word in a text. Assume for example that you want to know how often the term *the* or *house* occurs in all of Wikipedia. The result is a file that has a line for each term and its count.
...
This code shows how the plan for Word Count is created. You see a *source*
(FileDataSource), two *operators* (map and reduce), and a *sink* (CsvOutputFormat).

The operations are defined by specialized classes, here the *SplitWords* and
*CountWords* classes. The following snippet shows the implementation of the *SplitWords* class:
...
For the full example code, [check it out on GitHub]
(https://github.com/stratosphere/stratosphere/blob/release-0.4-rc1/stratosphere-examples/stratosphere-java-examples/src/main/java/eu/stratosphere/example/java/record/wordcount/WordCount.java?source=cc).
