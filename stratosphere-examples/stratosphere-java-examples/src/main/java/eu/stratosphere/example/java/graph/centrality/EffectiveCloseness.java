...
public class EffectiveCloseness {
	 ...
	 * solution-set
	 */
	DataSet<Tuple3<Long, VertexWithBitArray, Double>> initialworkingSet =
			initialSolutionSet;

	/**
	 * Reads Edges input file of the format <sourceId, destinationId>
	 */
	DataSet<Tuple2<Long, Long>> edgeSet = env.readCsvFile(edgeInput)
		.fieldDelimiter(delim).types(Long.class, Long.class);
	...
	Tuple3<Long, VertexWithBitArray, Double>> iteration = initialSolutionSet
	 ...
	 * <des, bit, sum>
	 *
	 * groupby des and computes count of all neighbors of that des node by
	 * using merge method {no need to merge current vertex's node in the
	 * sum} and emit <des, updatedbit, updatedsum>
	 *
	 * joine solutionset <vertexId,bit,sum> with previous step on (vertexId
	...
	DataSet<Tuple2<Long, Double>> closeness = finalSolnSet
	   ...
	   // closeness.writeAsCsv(output, "\n", TAB_DELIM);
	CsvOutputFormat<Tuple2<Long,Double>> csvOut = new CsvOutputFormat
			<Tuple2<Long,Double>>(new Path(output), NEWLINE, TAB_DELIM);
	csvOut.setWriteMode(WriteMode.OVERWRITE);
	closeness.output(csvOut).name("Writing Results");
	env.execute();
	...
	public AverageComputation(String numVer) {
		this.noOfver = Integer.parseInt(numVer);
	}
	@Override
	...
	public void reduce(
		...
		firstInList.merge(tuple.f1.getFmCounter());// // BITWISE OR of
								// current node with
								// initial record in
								// the iterator
		}
	 ...
	 *
	 */
	public static class VertexWithBitArray {
