...
public class TypeExtractor {
	...
	public static <X> TypeInformation<X> getMapReturnTypes(MapFunction<?, X> mapFunction) {
		...
		return new TypeExtractor().privateCreateTypeInfo(returnType);
	}
	...
	private <X> TypeInformation<X> privateCreateTypeInfo(Type t) {
		...
		if ((curT instanceof Class<?> && Tuple.class.isAssignableFrom((Class<?>) curT)) ||
			...
			if(curT instanceof ParameterizedType) {
				...
				if (raw instanceof Class) {
					...
					for (int i = 0; i < subtypes.length; i++) {
						tupleSubTypes[i] = privateCreateTypeInfo(subtypes[i]);
					}
			...
			// non tuple
			return privateGetForClass((Class<X>) t);
		}
	...
	private <X> TypeInformation<X> privateGetForClass(Class<X> clazz) {
		...
		TypeInformation<X> arrayTypeInfo = ArrayTypeInfo.getInfoFor(clazz);
		...
		if (!containsReadObjectOrWriteObject && pojoType.isSupportedInPojo()) {
			return pojoType;
		}
	...
	private static ParameterizedType getTemplateTypesChecked(Class<?> baseClass, Class<?> clazz, int pos) {
